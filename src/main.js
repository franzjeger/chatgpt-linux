  ...
  target.webContents.on("context-menu", (_event, params) => {
    ...
    if (params.linkURL) {
      add({
        ...
        click: () =>
          isSafeExternal(params.linkURL) && shell.openExternal(params.linkURL),
      });
      add({
    ...
    Menu.buildFromTemplate([
      ...
      },
      {
        ...
        submenu: [
          ...
          {
            label: `Electron ${process.versions.electron} · Chromium ${process.versions.chrome}`,
            enabled: false,
          },
        ],
      },
...
function configureSession() {
  ...
  const permitted = (permission, url) =>
    ALLOWED.has(permission) && isInternal(url);

  ses.setPermissionRequestHandler((contents, permission, callback, details) => {
    callback(
      permitted(
        permission,
        (details && details.requestingUrl) || contents.getURL(),
      ),
    );
  });
  ses.setPermissionCheckHandler((_contents, permission, origin) =>
    permitted(permission, origin),
  );
  ...
  try {
    ...
    try {
      ...
      /* no dictionary available at all */
    }
  }
}
...
function registerShortcut() {
  ...
  try {
    ...
    console.warn(
      `shortcut: ${accelerator} is not a valid accelerator —`,
      err.message,
    );
  }
}
